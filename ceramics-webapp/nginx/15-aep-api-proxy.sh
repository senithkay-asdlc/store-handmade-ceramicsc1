...
# Primary sibling address: ceramics-api, the only component-kind dependency.
# OpenChoreo injects CERAMICS_API_URL on the pod (visibility: project).
API_URL="${CERAMICS_API_URL:-}"
API_BACKEND="$(echo "${API_URL}" | sed 's|^https\{0,1\}://||' | sed 's|/.*||')"
if [ -z "$API_BACKEND" ]; then
    echo "aep-api-proxy: CERAMICS_API_URL unset; /api will 502 until OpenChoreo injects it"
